...
<div class="row">
    <div class="col-md-8 my-2 px-2">
        <div class="cd--one">
            <div class="cd-header">
                <h1> Editar Imóvel - {{$immobile->slug}}</h1>
            </div>
            <div class="m-1">
                ...
                <p>{{$immobile->slug}}<br>
                    {{$immobile->min_description}}</p>
                <hr>
                <p class="text-success">{{session('success')}}</p>
                <p class="text-danger">{{session('error')}}</p>
                @include('laraveladmin::parts.generateform',['form'=>view('admin.immobiles.parts.formedit',['immobile'=>$immobile,'neighborhoods'=>$neighborhoods,'types'=>$types])->render()])
            </div>
        </div>
    </div>
